...
bool CGeoPdfColorFinder::SetColors(ColorType colorType, std::vector<long> components)
{
	...
	if (colorType == ColorType::rgb)
	{
		...
		double m = (1. - g - k)/(1. - k);
		double y = (1. - b - k)/(1. - k);
		m_cmykComponents.push_back((long)(c*100 + 0.6));
		m_cmykComponents.push_back((long)(m*100 + 0.6));
		m_cmykComponents.push_back((long)(y*100 + 0.6));
		m_cmykComponents.push_back((long)(k*100 + 0.6));
	}
	else
	{
		size_t sz = components.size();
...
bool CGeoPdfColorFinder::ParseTreeElement(PDSElement elemlement)
{
	ASInt32 attrsNum = PDSElementGetNumAttrObjs(elemlement);
	ASInt32 count = PDSElementGetNumKids(elemlement);
	if (count < 1)
		return true; // no kid, go forward
	if (count > 1)
	{
		// It's not leaf node, go in depth
		ParseStructElement(elemlement, count);
		return true;
	}
	CosObj kid;
	PDSMC cont;
	...
	ASAtom  typ = PDSElementGetKidWithMCInfo(elemlement, 0, &kid, &mcInfo, (void**)&cont, NULL);
	...
	if (attrsNum > 0)
	{
		...
		m_lastError = GetObjId(elemlement, idVal);
		if (m_lastError != MyErrorCodes::NoError)
			return false;
	}
	...
	ASBool isRoot;
	PDSElementGetParent(elemlement, &parentElem, &isRoot);
...
void CGeoPdfColorFinder::ParseStructElement(CosObj elem, ASInt32 count)
{
	for (ASInt32 i = 0; i < count; i++)
	{
